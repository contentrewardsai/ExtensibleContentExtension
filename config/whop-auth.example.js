 ...
 * getLoginUrl(code, extId, intent): the side panel generates a one-time nonce and passes it as ?code=<nonce>,
 * plus the extension id as ?ext_id=<chrome.runtime.id>. Optional `intent` (e.g. `trial`) is copy-only;
 * OAuth is the same. The login page must carry the code through the
...
(function (global) {
  ...
  function getLoginUrl(code, extId, intent) {
    ...
    const i = intent == null ? '' : String(intent).trim();
