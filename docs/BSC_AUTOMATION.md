...
- **`pancakeV3RangeWatch`** — tab poll until V3 tick exits position range, or one-shot when `waitUntilOutOfRange: false`; **`CFS_BSC_V3_RANGE_CHECK`**. Always-on ticks apply a Multicall snapshot in-worker (`swTick`).
- **`reconcileV3Positions`** — NPM vs `alwaysOn.boundRows` (`CFS_V3_RECONCILE_POSITIONS`).
