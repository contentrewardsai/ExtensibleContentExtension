...
| **L2** | Read-only RPC reachability | `npm run test:crypto-rpc-smoke` — Solana `getEpochInfo` + …; EVM `net_version` ↔ `eth_chainId`, `eth_syncing`, `eth_gasPrice`, …; BSC **56**: `eth_call` WBNB `decimals()`; see [CRYPTO_CI_SMOKE.md](./CRYPTO_CI_SMOKE.md) |
