...
- **Per-workflow:** Add a **`checkRealtimeData`** step (Plan → Steps). **Always-on (background)** on that step (`alwaysOnEnabled`) pauses or resumes without clearing sources (auto-saves like Activity **Enabled**). Activity poll / NFT / policy fields need **Save rules**. The side panel and MCP **`set_always_on_scope`** derive **`alwaysOn.enabled` / `alwaysOn.scopes`** from that step (merge, never replace **`boundRows`**; scope is optional when only pausing). **`alwaysOn.conditions`** can require a non-empty Following bundle or a BSC indexer key. Activity **Real-time data** lists unique shared feeds and one **always-on workflow** row per family. V3 **Data** is **`alwaysOn.boundRows`** (same table in Plan and MCP `list_always_on_workflows` / `set_always_on_data_row`). Per-row in-range / last-tick on Activity are derived from the last poll. Library copies of the V3 monitor are upgraded in place (check / gas / reconcile / one-shot range) on Activity open or the next tick.
- **V3 LP ticks** (`background/v3-range-watch.js`): shared Multicall, then SW-safe prefix in-worker (`meta.swTick`: range / gas / reconcile). A tab opens only for matching **`runWorkflow`** children. `onOutOfRange` JSON is not the live router when those children exist.
- **Does not replace** manual Run or scheduled runs (those still open a tab and use **`PLAYER_START`**). Playback of **`checkRealtimeData`** only reads the latest snapshot.
- **Service worker:** **`shared/cfs-always-on-automation.js`** (loaded before **`background/solana-watch.js`** / **`background/bsc-watch.js`**) implements **`__CFS_evaluateFollowingAutomation`**. Collect uses the **highest `version` per `initial_version` family** and skips **`_testOnly`**. If the Library has workflows but **none** use always-on, the extension keeps **legacy** behavior: Following is allowed when any workflow exists. If **any** workflow enables always-on, only merged scopes enable **`cfs_solana_watch_poll`** / **`cfs_bsc_watch_poll`** (and file / LP / custom pollers). Custom HTTP/WS sockets live in the service worker (not offscreen); HTTP poll interval is **≥ 30s**.
- **Following bundles stay shared.** Two workflows that check Solana Following still cause **one** Solana tick (`cfsPulseSolanaWatchBundle`).
- **Security:** Following automation still respects **`cfsFollowingAutomationGlobal`** (pause, paper mode, denylists) and per-wallet Pulse settings; always-on metadata does not embed executable code. MCP **`subscribe`** is process-local and is not these SW feeds.
