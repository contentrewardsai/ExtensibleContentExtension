...
**Chrome “content scripts”** are the JS files the extension injects into every matching tab. The manifest **`content_scripts[0].js`** array defines **load order** for the main-frame tab bundle. The canonical ordered list is **`shared/content-script-tab-bundle.js`** (`CFS_CONTENT_SCRIPT_TAB_BUNDLE_FILES` only — no concatenated sources; the service worker imports the file list, not content-script bodies). It must match **`manifest.json`**. Validate with **`npm run check:content-bundle`**. **Scheduled runs** and the side panel reinject the same bundle via that list.

Load order: **shared/selectors.js** → **shared/recording-value.js** → **shared/selector-parity.js** → **shared/manifest-loader.js** → **shared/template-resolver.js** → **shared/row-list-normalize.js** → **shared/run-if-condition.js** → **shared/project-id-resolve.js** → **shared/personal-info-sync.js** → **shared/removed-step-types.js** → **steps/registry.js** → **steps/loader.js** (fetches **steps/manifest.json**; background injects each **steps/{id}/handler.js**) → **content/recorder.js** → **content/player.js** → **shared/discovery-input-normalize.js** → **content/auto-discovery.js**.
...
| **shared/row-list-normalize.js** | Row list helpers (`mergedEvalRow`, `sliceResult`, list normalization) for filter/join/concat/dedupe steps. |
| **shared/run-if-condition.js** | Shared **`runIf`** evaluation (`CFS_runIfCondition`) for player and steps that re-check conditions. |
| **shared/project-id-resolve.js** | Resolve uploads **`projectId`** from row, Library default, or fallback. |
| **shared/personal-info-sync.js** | Personal-info field sync helpers for workflow rows. |
| **shared/removed-step-types.js** | Legacy step type ids (generator, Upload Post, etc.) for migration and guards. |
| **shared/discovery-input-normalize.js** | Normalize discovery hint inputs for auto-discovery. |
...
|-----------|---------|-----------------------------|
| **background/** | Service worker (message routing, offscreen docs, downloads). Shared crypto/RPC helpers: **`crypto-storage.js`** (`CFS_CRYPTO_STORAGE`), **`solana-rpc-helpers.js`** (`CFS_SOLANA_RPC`), **`evm-helpers.js`** (`CFS_EVM_HELPERS`) — loaded via **`importScripts`** before consumers. Also loads **`aster-futures.js`** (Aster REST, **`CFS_ASTER_FUTURES`**, IP **request-weight** + account **order-count** pacing from **`exchangeInfo`** / response headers), **`solana-sellability-probe.js`** (**`CFS_SOLANA_SELLABILITY_PROBE`**), **`bsc-sellability-probe.js`** (**`CFS_BSC_SELLABILITY_PROBE`**), **`crypto-test-wallets.js`** (**`CFS_CRYPTO_TEST_ENSURE_WALLETS`** — devnet/Chapel labeled test wallets; see **`docs/TESTING.md`**), **`following-automation-runner.js`** (Pulse Following automation headless pipeline), and other domain modules. Prebuilt bundles (commit after `npm run build:*`): **`evm-lib.bundle.js`** (ethers for **`bsc-evm.js`**), **`infinity-sdk.bundle.js`** (Pancake Infinity SDK before **`bsc-evm.js`**), plus Solana/Raydium/Meteora/Pump bundles as listed in **`package.json`**. | Yes – manifest `background.service_worker` points here. |
| **sidepanel/** | Side panel UI (workflows, recording, playback). **`media-grant.js`** for mic/webcam grant pages; **`devnet-smoke-utils.js`** loaded from **`sidepanel.html`** before step sidepanels. | Yes – manifest `side_panel.default_path` points here. |
...
| **shared/** | Code used by more than one context (selectors, analyzer, backend, walkthrough-export). **`shared/dom-utils.js`** — `escapeHtml` / `escapeAttr` for UI. **`shared/playback-error-normalize.js`** — playback error message normalization. **`shared/cfs-always-on-automation.js`** — Pulse Following workflow gate + **`alwaysOn`** scope merge; loaded by the service worker before **`background/solana-watch.js`**. | Yes – shared libraries, not entry points. |
...
| **VIDEO_PROBE_API.md** | Video metadata probing (archived; unified editor removed). |
