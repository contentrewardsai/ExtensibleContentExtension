...
| `CRYPTO_HTTP_SMOKE=1 npm run test:crypto-http-smoke` | Optional — Rugcheck + Aster public ping/time (and BscScan if `CRYPTO_HTTP_SMOKE_BSCSCAN_API_KEY` set) |
...
**Optional CI secrets:** `SOLANA_RPC_SMOKE_URL`, `BSC_RPC_SMOKE_URL`, `CRYPTO_SOLANA_TX_SECRET_KEY`, `CRYPTO_EVM_FORK_RPC_URL`, `CRYPTO_HTTP_SMOKE_RUN`, `CRYPTO_HTTP_SMOKE_BSCSCAN_API_KEY` — see [CRYPTO_CI_SMOKE.md](./CRYPTO_CI_SMOKE.md). RPC smoke runs if **either** Solana or BSC URL is set.
