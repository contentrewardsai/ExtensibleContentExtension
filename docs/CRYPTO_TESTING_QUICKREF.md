...
| **`CFS_CRYPTO_TEST_ENSURE_WALLETS`** on **`test/unit-tests.html`** (**Run crypto tests** / **Request test tokens again** / **Replace crypto test wallets**) | Create or reuse devnet + Chapel test wallets; payload **`fundOnly`**, **`replaceExisting`** (not together — see **`docs/TESTING.md`**). Open the page from **Settings → Tests → Open unit tests page**, or Playwright **`unit-tests.spec.mjs`**. Settings **Crypto test wallets** uses **`data-testid`** **`cfs-settings-crypto-*`**; shared constants: **`test/e2e/cfs-e2e-testids.mjs`**. See **`docs/TESTING.md`** **Stable selectors**. Playwright: **`E2E_CRYPTO_ENSURE_TEST_WALLETS=1`**, **`PW_UNIT_CRYPTO_ENSURE=1`**. **`npm run test:unit`** (Puppeteer, `file://`) has no extension — cannot ensure |
