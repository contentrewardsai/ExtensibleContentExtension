...
- **Playwright crypto E2E (opt-in):** `npm run test:e2e:crypto` with **`E2E_CRYPTO=1`** — loads the unpacked extension and sends **`CFS_SOLANA_RPC_READ`** (mintInfo, nativeBalance, tokenBalance), **`CFS_BSC_QUERY`** (rpcInfo, isContract, erc20Metadata on chain 56), **`CFS_ASTER_FUTURES`** (futures + spot public), **`CFS_RUGCHECK_TOKEN_REPORT`** over **`chrome.runtime`** (see **`test/e2e/crypto-e2e-playwright.spec.mjs`**). Set **`SOLANA_RPC_SMOKE_URL`** / **`BSC_RPC_SMOKE_URL`** (or **`E2E_CRYPTO_*_RPC_URL`**) for on-chain reads. Optional CI: repository secret **`E2E_CRYPTO_PLAYWRIGHT=1`** plus the same RPC URLs — job **`optional-e2e-crypto-playwright`** in **`.github/workflows/extension-checks.yml`**.
