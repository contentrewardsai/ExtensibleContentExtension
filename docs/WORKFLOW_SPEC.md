...
- **`alwaysOn` (optional)** – Per-workflow opt-in for background automation in the service worker (not tab playback). When present, **`alwaysOn.enabled`** toggles background participation; **`alwaysOn.scopes`** selects Solana/BSC Following watch, Following automation, **file watch**, **price range watch** (dual-mode: Infinity bin LP or Pancake V3 NFT on BSC — see **`docs/BSC_INFI_LP_WORKFLOWS.md`** / **`docs/BSC_V3_LP_WORKFLOWS.md`**), or custom triggers; **`alwaysOn.conditions`** can require a non-empty Following bundle or an Etherscan (BSC Multichain) API key for BSC. **`alwaysOn.priceRangeWatch`** holds monitor fields and **`onOutOfRange`** rules (`runIf` + **`workflowId`**) that trigger child workflows via background tab playback. If **no** workflow in Library sets **`alwaysOn.enabled`**, the extension uses **legacy** gating: any non-empty Library is enough to allow Following. If **any** workflow enables **`alwaysOn`**, only merged scopes enable polling and automation. Configure in the side panel under **Library → Background automation**. See **`shared/cfs-always-on-automation.js`**, **`docs/SOLANA_AUTOMATION.md`** (Pulse), and **`docs/PLUGIN_ARCHITECTURE.md`**.
