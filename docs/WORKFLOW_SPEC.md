...
- **Step types:** click, type, select, ensureSelect, upload, download, wait, watchVideoProgress, waitForVideos, checkCompletions, **checkSuccessfulGenerations**, **delayBeforeNextRun**, runWorkflow, loop, **ifCondition**, etc.
- **`runIf` (every step):** optional comparison, e.g. `{{exitPolicy}} === restake`. The player evaluates `runIf` **before** `loop` / `runWorkflow` / `waitForElement` / navigation. Complementary child workflows (exit vs restake) each carry a `runIf`; only the matching child runs. Always-on `onOutOfRange` rules use the same evaluator (fail-closed if the SW helper is missing).
- **`ifCondition`:** authored or analyzer-generated branch container: `{ condition, thenSteps, elseSteps }`. Arms are usually `runWorkflow` (or recorded clicks). Playback walks the chosen arm with the same runner. v1 does not nest `ifCondition` inside `ifCondition`.
- **Analyzer branches:** After aligning runs, complementary optional clusters can be wrapped in `ifCondition` (Analyze UI: **Create if/else from optional clusters**). A single optional click that `trySkipByDOMState` already covers stays optional.
- **Flow view:** The side panel List / Flow toggle draws an SVG flowchart from `analyzed.actions`, nested `loop.steps` / `ifCondition` arms, `runWorkflow` calls, and always-on targets. **Chain workflow** appends a `runWorkflow` step. The chart is a **view** — playback still uses the flat `analyzed.actions[]` list.
- **Page-state booleans:** `waitForElement` can `saveAsVariable` + `saveResult` (`found` | `notFound`) so a later `runIf` can test `{{loginVisible}} === true`.
...
- **`alwaysOn` (optional, derived)** – Background participation is derived from **`checkRealtimeData`** steps (`shared/cfs-always-on-automation.js`). **`alwaysOnEnabled`** on that step (default on if missing) pauses or resumes without clearing sources; derived **`alwaysOn.enabled`** is the step switch **and** at least one source. On save, the extension **merges** **`alwaysOn.enabled`**, **`alwaysOn.scopes`**, and **`alwaysOn.conditions`** onto the workflow object without wiping **`boundRows` / `boundRow` / `priceRangeWatch` / gas / reconcile**. **`alwaysOn.scopes`** selects Solana/BSC Following watch, Following automation, **file watch**, **price range watch** (Pancake V3 / Infinity / Raydium CLMM / Meteora DLMM — see **`docs/BSC_INFI_LP_WORKFLOWS.md`** / **`docs/BSC_V3_LP_WORKFLOWS.md`** / **`docs/SOLANA_AUTOMATION.md`**), or custom HTTP/WS. V3 monitor **child routing is Plan `runWorkflow` + `runIf` steps**; **`alwaysOn.priceRangeWatch.onOutOfRange`** is legacy JSON compiled into steps only when those children are missing. Always-on **Data** for a price-range monitor is **`alwaysOn.boundRows`** (Activity table, Plan Data, MCP `list_always_on_workflows` / `set_always_on_data_row`). If **no** workflow sets **`alwaysOn.enabled`**, Following uses **legacy** gating (any Library workflow). If **any** workflow enables always-on, only merged scopes from the **latest version per `initial_version` family** poll. Configure by adding **Check for real-time data** in Plan → Steps. Activity lists unique shared feeds and one always-on workflow row per family. Following address-book bundles **`cfsPulseSolanaWatchBundle` / `cfsPulseBscWatchBundle`** remain the only Solana/BSC watch input. See **`shared/realtime-feeds.js`** and **`docs/PLUGIN_ARCHITECTURE.md`**.
