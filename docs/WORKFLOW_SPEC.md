...
- **Graph view:** The side panel list/graph toggle derives nodes and edges from `analyzed.actions`, nested `loop.steps` / `ifCondition` arms, `runWorkflow` calls, and always-on targets. The graph is a **view** — playback still uses the flat `analyzed.actions[]` list.
