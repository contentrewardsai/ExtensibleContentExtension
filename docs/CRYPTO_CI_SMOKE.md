...
## HTTP smoke (Rugcheck, Aster, Jupiter quote, optional Etherscan V2)

Read-only **`GET`** checks aligned with **`following-automation-runner.js`** (Rugcheck), **`aster-futures.js`** (public ping/time), and **`solana-swap.js`** / Jupiter quote hosts. Optional **Etherscan Multichain V2** proxy **`eth_blockNumber`** when an API key secret is set (same pattern as **`bsc-watch.js`**). Free keys that reject BSC **`chainid`** coverage soft-skip that check (exit 0) with a log line.
...
| **`CRYPTO_HTTP_SMOKE_BSCSCAN_API_KEY`** | Etherscan Multichain V2 API key — proxy **`eth_blockNumber`** via **`api.etherscan.io/v2/api`** (**`chainid=56`**, or **`97`** when **`CRYPTO_HTTP_SMOKE_BSCSCAN_NETWORK=chapel`**). Free keys may lack BSC coverage. |
