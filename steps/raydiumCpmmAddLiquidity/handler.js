(function() {
  ...
  window.__CFS_registerStepHandler('raydiumCpmmAddLiquidity', async function(action, opts) {
    const ctx = opts && opts.ctx;
    if (!ctx) throw new Error('Step context missing (raydiumCpmmAddLiquidity)');
    ...
    if (!poolId || !amountInRaw) throw new Error('Raydium CPMM add: set poolId and amountInRaw.');
    if (fixedSide !== 'a' && fixedSide !== 'b') throw new Error('fixedSide must be a or b');
    ...
    if (!response || !response.ok) {
      const err = (response && response.error) ? response.error : 'Raydium CPMM add liquidity failed';
