/**
 * Pump.fun bonding-curve buy: spend SOL (lamports) for tokens. Not for graduated coins.
 */
(function() {
  'use strict';
  ...
  window.__CFS_registerStepHandler('solanaPumpfunBuy', async function(action, opts) {
    const ctx = opts && opts.ctx;
    if (!ctx) throw new Error('Step context missing (solanaPumpfunBuy)');
    ...
    if (!mint || !solLamports) {
      throw new Error('Pump.fun buy: set token mint and SOL to spend (solLamports, integer string).');
    }
    ...
    if (!response || !response.ok) {
      const err = (response && response.error) ? response.error : 'Pump.fun buy failed';
