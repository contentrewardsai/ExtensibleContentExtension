(function() {
  ...
  window.__CFS_registerStepHandler('meteoraCpammClaimReward', async function(action, opts) {
    const ctx = opts && opts.ctx;
    if (!ctx) throw new Error('Step context missing (meteoraCpammClaimReward)');
    ...
    let rewardIndexRaw = resolveTemplate(
      String(action.rewardIndex != null ? action.rewardIndex : '0').trim(),
      row,
      getRowValue,
      action
    ).trim();
    const ri = parseInt(rewardIndexRaw, 10);
    const rewardIndex = Number.isFinite(ri) && ri >= 0 && ri <= 1 ? ri : 0;
    const isSkipReward = action.isSkipReward === true;
    ...
    if (!position) throw new Error('Meteora CP-AMM claim incentive: set position.');
    ...
    if (!response || !response.ok) {
      const err = (response && response.error) ? response.error : 'Meteora CP-AMM claim incentive failed';
