/**
 * Sell token for SOL: Pump.fun bonding curve if still active, otherwise Jupiter (mint -> WSOL).
 */
(function() {
  'use strict';
  ...
  window.__CFS_registerStepHandler('solanaPumpOrJupiterSell', async function(action, opts) {
    const ctx = opts && opts.ctx;
    if (!ctx) throw new Error('Step context missing (solanaPumpOrJupiterSell)');
    ...
    if (!mint || !tokenAmountRaw) {
      throw new Error('Pump or Jupiter sell: set mint and tokenAmountRaw (amount in token smallest units).');
    }
    ...
    if (!response || !response.ok) {
      const err = (response && response.error) ? response.error : 'Sell failed';
