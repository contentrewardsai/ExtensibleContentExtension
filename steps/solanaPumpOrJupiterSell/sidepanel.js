(function() {
  ...
  window.__CFS_registerStepSidepanel('solanaPumpOrJupiterSell', {
    label: 'Solana Pump or Jupiter sell',
    defaultAction: {
    ...
    getSummary: function(action) {
      ...
      return m ? 'Sell ' + m.slice(0, 8) + '…' + (a ? ' ' + a.slice(0, 12) : '') : 'Solana Pump or Jupiter sell';
    },
    ...
    renderBody: function(action, i, wfId, totalCount, helpers) {
      ...
      var body =
        '<p class="step-hint">Probes Pump curve, then Pump sell or Jupiter mint→WSOL.</p>' +
        ...
        '<div class="step-field"><label>Token amount (raw)</label><input type="text" data-field="tokenAmountRaw" data-step="' + i + '" value="' + escapeHtml((action.tokenAmountRaw || '').toString()) + '"></div>' +
        ...
        '<div class="step-field"><label>Save venue var</label><input type="text" data-field="saveVenueVariable" data-step="' + i + '" value="' + escapeHtml((action.saveVenueVariable || 'sellVenue').toString()) + '"></div>' +
      ...
      return window.__CFS_buildStepItemShell('solanaPumpOrJupiterSell', action, i, totalCount, helpers, body);
    },
    saveStep: function(item, action, idx) {
      ...
      var out = { type: 'solanaPumpOrJupiterSell' };
