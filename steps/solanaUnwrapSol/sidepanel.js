(function() {
  ...
  window.__CFS_registerStepSidepanel('solanaUnwrapSol', {
    label: 'Solana unwrap WSOL',
    defaultAction: {
      type: 'solanaUnwrapSol',
      runIf: '',
    ...
    getExtraVariableKeys: function(action) {
      ...
      var ar = (action.saveAmountRawVariable || '').trim();
      if (ar) out.push({ rowKey: ar, label: ar, hint: 'amount raw before close' });
    ...
    renderBody: function(action, i, wfId, totalCount, helpers) {
      ...
      var saveAmt = (action.saveAmountRawVariable || '').trim();
      var saveSig = (action.saveSignatureVariable || '').trim();
      var saveExp = (action.saveExplorerUrlVariable || '').trim();

      var body =
        '<p class="step-hint" style="margin-bottom:10px;">Closes your WSOL ATA (unwrap wrapped SOL + reclaim rent). ATA must exist. Uses <strong>Settings → Solana automation</strong>.</p>' +
        ...
        '<div class="step-field"><label>Save amount raw to variable</label><input type="text" data-field="saveAmountRawVariable" data-step="' + i + '" value="' + escapeHtml(saveAmt) + '"></div>' +
      ...
      return window.__CFS_buildStepItemShell('solanaUnwrapSol', action, i, totalCount, helpers, body);
    },
    saveStep: function(item, action, idx) {
      ...
      out.saveAmountRawVariable = (getVal('saveAmountRawVariable') || '').trim();
