(function() {
  ...
  window.__CFS_registerStepSidepanel('raydiumClmmSwap', {
    label: 'Raydium swap (CLMM)',
    defaultAction: {
    ...
    getSummary: function(action) {
      ...
      return p ? 'Raydium CLMM swap ' + p.slice(0, 8) + '…' : 'Raydium swap (CLMM)';
    },
    ...
    renderBody: function(action, i, wfId, totalCount, helpers) {
      ...
      var body =
        '<p class="step-hint">CLMM concentrated pool only; mints must match pool legs. WSOL uses wrapped mint.</p>' +
        ...
        '<div class="step-field"><label>Min out raw (override)</label><input type="text" data-field="amountOutMinRaw" data-step="' + i + '" value="' + escapeHtml((action.amountOutMinRaw || '').toString()) + '"></div>' +
      ...
      return window.__CFS_buildStepItemShell('raydiumClmmSwap', action, i, totalCount, helpers, body);
    },
