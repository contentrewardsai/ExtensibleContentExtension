(function() {
  ...
  window.__CFS_registerStepHandler('raydiumClmmSwap', async function(action, opts) {
    const ctx = opts && opts.ctx;
    if (!ctx) throw new Error('Step context missing (raydiumClmmSwap)');
    ...
    if (!poolId || !inputMint || !outputMint || !amountInRaw) {
      throw new Error('Raydium CLMM swap: set poolId, inputMint, outputMint, and amountInRaw.');
    }
    ...
    if (!response || !response.ok) {
      const err = (response && response.error) ? response.error : 'Raydium CLMM swap failed';
