(function() {
  ...
  window.__CFS_registerStepSidepanel('runGenerator', {
    ...
    renderBody: function(action, i, wfId, totalCount, helpers) {
      ...
      if (templateIds.indexOf(pluginId) === -1 && templateIds.length) pluginId = templateIds[0];
      ...
      var optionsHtml = templateIds.map(function(p) {
        return '<option value="' + escapeHtml(p) + '"' + (p === pluginId ? ' selected' : '') + '>' + escapeHtml(p) + '</option>';
      }).join('');
      ...
      var body =
        ...
        '<div class="step-field"><label>Generator (template)</label><select data-field="pluginId" data-step="' + i + '">' + optionsHtml + '</select></div>' +
