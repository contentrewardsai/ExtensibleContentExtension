(function() {
  ...
  window.__CFS_registerStepSidepanel('readJsonFromProject', {
    label: 'Read JSON from project file',
    defaultAction: {
      type: 'readJsonFromProject',
      runIf: '',
      relativePath: 'data/example.json',
      saveAsVariable: 'fileJson',
    ...
    renderBody: function(action, i, wfId, totalCount, helpers) {
      ...
      var body =
        ...
        '<div class="step-field"><label>Relative path</label><input type="text" data-field="relativePath" data-step="' + i + '" value="' + escapeHtml(rel) + '" placeholder="data/state.json"></div>' +
        '<div class="step-field"><label>Save as variable</label><input type="text" data-field="saveAsVariable" data-step="' + i + '" value="' + escapeHtml(saveAs) + '" placeholder="fileJson"></div>' +
        '<div class="step-field"><label>Project ID row key</label><input type="text" data-field="projectIdVariableKey" data-step="' + i + '" value="' + escapeHtml(pidKey) + '" placeholder="projectId"></div>' +
        ...
        '<option value="empty"' + (ifMissing === 'empty' ? ' selected' : '') + '>Set empty object {}</option>' +
        '<option value="skip"' + (ifMissing === 'skip' ? ' selected' : '') + '>Skip (no-op)</option>' +
        '</select></div>' +
        '<div class="step-field"><label>Max read bytes (optional)</label><input type="number" data-field="maxBytes" data-step="' + i + '" value="' + escapeHtml(maxB) + '" placeholder="5242880" min="1"></div>' +
        '<div class="step-actions"><button class="btn btn-primary" data-save-step="' + i + '">Save</button></div>';
      return window.__CFS_buildStepItemShell('readJsonFromProject', action, i, totalCount, helpers, body);
    },
    saveStep: function(item, action, idx) {
      ...
      var out = { type: 'readJsonFromProject' };
      ...
      var mb = getVal('maxBytes');
      if (mb !== undefined && mb !== '' && mb != null) {
        var n = parseInt(mb, 10);
        if (!isNaN(n) && n > 0) out.maxBytes = n;
      }
      return out;
    },
  });
