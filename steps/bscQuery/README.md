...
| `v3PoolState` | **`v3Pool`** (pool contract) | `{ pool, token0, token1, fee, tickSpacing, liquidity, sqrtPriceX96, tick, priceToken1PerToken0, priceToken0PerToken1, feeGrowthGlobal0X128, feeGrowthGlobal1X128, …slot0… }` |
| `v3PriceTicks` | **`v3Pool`** (or **`decimals0`/`decimals1`/`tickSpacing`**) + **`minPrice`+`maxPrice`**, or **`price`**, or **`tick`**; optional **`priceDenomination`** (`token1PerToken0` default = Pancake UI “token1 per 1 token0”) | Spacing-aligned ticks / human prices for a position range |
| `v3LiquidityDepth` | **`v3Pool`**; optional **`wordRange`** (default 1), **`maxTicks`** | TickLens ticks with **`priceToken1PerToken0`**, **`liquidityGross`/`Net`**, **`activeLiquidityAbove`**; optional subgraph TVL/volume when `cfs_thegraph_api_key` is set |
