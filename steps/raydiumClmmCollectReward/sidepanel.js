(function() {
  ...
  window.__CFS_registerStepSidepanel('raydiumClmmCollectReward', {
    label: 'Raydium CLMM collect reward',
    defaultAction: {
      type: 'raydiumClmmCollectReward',
      runIf: '',
      poolId: '',
      rewardMint: '',
    ...
    getSummary: function(action) {
      ...
      return p ? 'CLMM reward ' + p.slice(0, 6) + '…' : 'Raydium CLMM collect reward';
    },
    ...
    renderBody: function(action, i, wfId, totalCount, helpers) {
      ...
      var body =
        '<p class="step-hint">One reward mint per run. Mint must match pool farming rewards.</p>' +
        ...
        '<div class="step-field"><label>Reward mint</label><input type="text" data-field="rewardMint" data-step="' + i + '" value="' + escapeHtml((action.rewardMint || '').toString()) + '"></div>' +
      ...
      return window.__CFS_buildStepItemShell('raydiumClmmCollectReward', action, i, totalCount, helpers, body);
    },
    saveStep: function(item, action, idx) {
      ...
      var out = { type: 'raydiumClmmCollectReward' };
      var r = (getVal('runIf') || '').trim();
      if (r) out.runIf = r;
      out.poolId = (getVal('poolId') || '').trim();
      out.rewardMint = (getVal('rewardMint') || '').trim();
