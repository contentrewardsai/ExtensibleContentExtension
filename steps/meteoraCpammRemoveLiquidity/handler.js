(function() {
  ...
  window.__CFS_registerStepHandler('meteoraCpammRemoveLiquidity', async function(action, opts) {
    const ctx = opts && opts.ctx;
    if (!ctx) throw new Error('Step context missing (meteoraCpammRemoveLiquidity)');
    ...
    if (!position) throw new Error('Meteora CP-AMM remove: set position (from add step).');
    ...
    if (!response || !response.ok) {
      const err = (response && response.error) ? response.error : 'Meteora CP-AMM remove liquidity failed';
