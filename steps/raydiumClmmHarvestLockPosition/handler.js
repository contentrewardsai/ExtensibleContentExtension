(function() {
  ...
  window.__CFS_registerStepHandler('raydiumClmmHarvestLockPosition', async function(action, opts) {
    const ctx = opts && opts.ctx;
    if (!ctx) throw new Error('Step context missing (raydiumClmmHarvestLockPosition)');
    const { getRowValue, currentRow, sendMessage } = ctx;
    const row = currentRow || {};

    let lockNftMint = resolveTemplate(String(action.lockNftMint || '').trim(), row, getRowValue, action).trim();
    ...
    if (!lockNftMint) {
      throw new Error('CLMM harvest lock: set lockNftMint (the lock NFT mint, not the position NFT).');
    }
    ...
    if (!response || !response.ok) {
      const err = (response && response.error) ? response.error : 'Raydium CLMM harvest lock failed';
