(function() {
  ...
  window.__CFS_registerStepHandler('meteoraCpammSwap', async function(action, opts) {
    const ctx = opts && opts.ctx;
    if (!ctx) throw new Error('Step context missing (meteoraCpammSwap)');
    ...
    let minimumAmountOutRaw = resolveTemplate(String(action.minimumAmountOutRaw != null ? action.minimumAmountOutRaw : '').trim(), row, getRowValue, action).trim();
    ...
    if (!pool || !inputMint || !outputMint || !amountInRaw) {
      throw new Error('Meteora CP-AMM swap: set pool, inputMint, outputMint, and amountInRaw.');
    }
    ...
    if (!response || !response.ok) {
      const err = (response && response.error) ? response.error : 'Meteora CP-AMM swap failed';
