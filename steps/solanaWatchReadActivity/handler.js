/**
 * Read recent Solana Following watch activity. Message: CFS_SOLANA_WATCH_GET_ACTIVITY.
 */
(function () {
  'use strict';
  ...
  window.__CFS_registerStepHandler('solanaWatchReadActivity', async function (action, opts) {
    var ctx = opts && opts.ctx;
    if (!ctx) throw new Error('Step context missing (solanaWatchReadActivity)');
    ...
    var response = await sendMessage({ type: 'CFS_SOLANA_WATCH_GET_ACTIVITY', limit: limit });
    if (!response || !response.ok) {
      throw new Error((response && response.error) ? String(response.error) : 'Solana watch read failed');
    }
    ...
    if (applyClientFilters) {
      var filterAddr = ((typeof CFS_templateResolver !== 'undefined' && CFS_templateResolver.resolveActionField) ? CFS_templateResolver.resolveActionField(action, 'filterAddress', row, getRowValue) : trimResolved(row, getRowValue, action, action.filterAddress)).trim();
      if (filterAddr) {
        activity = activity.filter(function (r) {
          return String(r.address || '').toLowerCase().indexOf(filterAddr.toLowerCase()) !== -1;
        });
      }
