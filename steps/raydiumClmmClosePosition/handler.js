(function() {
  ...
  window.__CFS_registerStepHandler('raydiumClmmClosePosition', async function(action, opts) {
    const ctx = opts && opts.ctx;
    if (!ctx) throw new Error('Step context missing (raydiumClmmClosePosition)');
    ...
    if (!positionNftMint) throw new Error('CLMM close: set positionNftMint. Liquidity must already be zero.');
    ...
    if (!response || !response.ok) {
      const err = (response && response.error) ? response.error : 'Raydium CLMM close position failed';
