/**
 * Meteora DLMM range watch (MV3 service worker). Shared poll per lbPair; OOR child workflows like V3.
 */
(function (global) {
  'use strict';

  var ALARM_NAME = 'cfs_dlmm_range_poll';
  var LAST_POLL_KEY = 'cfsDlmmRangeWatchLastPoll';
  ...
  function collectJobs(stored) {
    ...
    var raw = RF && typeof RF.collectDlmmRangeJobs === 'function' ? RF.collectDlmmRangeJobs(stored) : [];
    var jobs = [];
  ...
  function callRangeCheck(msg) {
    return new Promise(function (resolve) {
      ...
      if (typeof impl !== 'function') {
        resolve({ ok: false, error: 'DLMM range check not loaded' });
        return;
      }
  ...
  async function appendActivity(entry) {
    try {
      ...
      list.unshift(Object.assign({ ts: Date.now(), family: 'dlmm' }, entry || {}));
  ...
  async function triggerOor(stored, job, pos, check) {
    ...
    var direction = check.activeBinId > check.upperBinId ? 'above' : 'below';
    var triggerRow = Object.assign({}, pos.row || {}, {
      driftDirection: direction,
      activeBinId: check.activeBinId,
    ...
    for (var ri = 0; ri < rules.length; ri++) {
      ...
      var jk = job.workflowId + '|dlmm|' + pos.position;
    ...
    try {
      ...
      for (var ji = 0; ji < jobs.length; ji++) {
        ...
        for (var pi = 0; pi < job.positions.length; pi++) {
          ...
          var check = await callRangeCheck({ lbPair: pos.lbPair, position: pos.position });
          if (check && check.ok) {
  ...
  global.__CFS_dlmmRangeWatch_tick = tick;
  global.__CFS_dlmmRangeWatch_setupAlarm = setupAlarm;
  global.__CFS_dlmmRangeWatch_collectJobs = collectJobs;
